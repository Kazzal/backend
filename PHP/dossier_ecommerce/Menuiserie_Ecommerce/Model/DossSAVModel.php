...
class ModeleDossierSAV{
    ...
    public function lireDossierViaDate($date) {
        $this->connexion();
        $res = $this->idc->prepare("SELECT csf.* FROM T_D_customerservicefolder_csf csf inner join t_d_tickethistory_thi thi on csf.THI_ID WHERE thi.thi_interventiontime = ?");
        $res->execute($date);  
        return $res;
    }

    public function InsertDossier($csfStatus, $csfDescription)
    {
        $this->connexion();
