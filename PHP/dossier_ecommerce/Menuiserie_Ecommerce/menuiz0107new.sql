...
--
-- Hôte : 127.0.0.1
-- Généré le : ven. 01 juil. 2022 à 11:29
...
CREATE TABLE IF NOT EXISTS `t_d_diagsav_dsa` (
...
CREATE TABLE IF NOT EXISTS `t_d_movestock_mvs` (
...
CREATE TABLE IF NOT EXISTS `t_d_tickethistory_thi` (
  `thi_ID` int(11) NOT NULL,
  `thi_interventionTime` datetime NOT NULL,
  PRIMARY KEY (`thi_ID`),
  UNIQUE KEY `thi_ID` (`thi_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS `t_d_ticketsavdetail_tsd` (
  `tsd_ID` int(11) NOT NULL,
  `tsd_description` varchar(45) NOT NULL,
  `tsd_IP` varchar(15) NOT NULL,
  PRIMARY KEY (`tsd_ID`),
  UNIQUE KEY `tsd_ID` (`tsd_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
